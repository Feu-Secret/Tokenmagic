  ...
  return {
    ...
    server: {
      ...
      proxy: {
        ...
        [`^(/${s_PACKAGE_ID}/(images|fonts|assets|fx|gui|import|libs|lang|languages|packs|styles|templates|style.css))`]:
    ...
    plugins: [
      ...
      viteStaticCopy({
        targets: [
          ...
          {
            src: normalizePath(path.resolve(__dirname, './src/fx')) + '/[!.]*', // 1️
            dest: normalizePath(path.resolve(__dirname, `./dist/${s_MODULE_ID}/fx`)), // 2️
          },
          {
            src: normalizePath(path.resolve(__dirname, './src/gui')) + '/[!.]*', // 1️
            dest: normalizePath(path.resolve(__dirname, `./dist/${s_MODULE_ID}/gui`)), // 2️
          },
          {
            src: normalizePath(path.resolve(__dirname, './src/import')) + '/[!.]*', // 1️
            dest: normalizePath(path.resolve(__dirname, `./dist/${s_MODULE_ID}/import`)), // 2️
          },
          {
            src: normalizePath(path.resolve(__dirname, './src/libs')) + '/[!.]*', // 1️
            dest: normalizePath(path.resolve(__dirname, `./dist/${s_MODULE_ID}/libs`)), // 2️
          },
          {
